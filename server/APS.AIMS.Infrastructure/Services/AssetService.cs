...
public sealed class AssetService : IAssetService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (filter.CompanyId.HasValue)
            query = query.Where(asset => asset.CompanyId == filter.CompanyId.Value);

        if (filter.BranchId.HasValue)
            query = query.Where(asset => asset.BranchId == filter.BranchId.Value);

        if (filter.DepartmentId.HasValue)
            query = query.Where(asset => asset.DepartmentId == filter.DepartmentId.Value);

        if (filter.LocationId.HasValue)
            query = query.Where(asset => asset.CurrentLocationId == filter.LocationId.Value);

        if (filter.Status.HasValue)
            query = query.Where(asset => asset.Status == filter.Status.Value);

        if (filter.Condition.HasValue)
            query = query.Where(asset => asset.Condition == filter.Condition.Value);
    ...
    public async Task<AssetDto?> GetByAssetIdAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var normalized = TextNormalizer.Code(assetId);

        return await _dbContext.Assets
            .AsNoTracking()
            .Where(asset => asset.AssetId == normalized)
            .Select(AssetProjection.ToDto)
            .FirstOrDefaultAsync(cancellationToken);
    }

    public async Task<AssetDto?> GetByBarcodeAsync(
        string barcode,
        CancellationToken cancellationToken = default)
    {
        var normalized = TextNormalizer.Required(barcode);

        return await _dbContext.Assets
            .AsNoTracking()
            .Where(asset => asset.BarcodeValue == normalized)
            .Select(AssetProjection.ToDto)
            .FirstOrDefaultAsync(cancellationToken);
    }
    ...
    public async Task<AssetDto> CreateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ValidateProfile(request.Name, request.AcquisitionCost, request.Currency);

        await ValidateRelationshipsAsync(request, cancellationToken);

        var assetId = await GenerateAssetIdAsync(cancellationToken);
        var barcodeValue =
            TextNormalizer.Optional(request.BarcodeValue) ?? assetId;
        ...
        return await GetRequiredByIdAsync(asset.Id, cancellationToken);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        asset.ShortDescription = TextNormalizer.Optional(request.ShortDescription);
        asset.CategoryId = request.CategoryId;
        asset.SerialNumber = TextNormalizer.Optional(request.SerialNumber);
        asset.Manufacturer = TextNormalizer.Optional(request.Manufacturer);
        asset.Model = TextNormalizer.Optional(request.Model);
        asset.PartNumber = TextNormalizer.Optional(request.PartNumber);
        asset.LegacyAssetId = TextNormalizer.Optional(request.LegacyAssetId);
        asset.AcquisitionCost = request.AcquisitionCost;
        asset.Currency = TextNormalizer.CodeOrNull(request.Currency);
        ...
        string? currency)
    {
        ...
        var normalizedCurrency = TextNormalizer.Optional(currency);
    ...
    {
        var companyIsValid = await _dbContext.Companies
            .AsNoTracking()
            .AnyAsync(
                company =>
        ...
        var branchIsValid = await _dbContext.Branches
            .AsNoTracking()
            .AnyAsync(
                branch =>
        ...
        var locationIsValid = await _dbContext.AssetLocations
            ...
            .AnyAsync(
                location =>
                    location.Id == request.CurrentLocationId &&
        ...
        if (request.CurrentCustodianId.HasValue)
        {
            var custodianIsValid = await _dbContext.Employees
                ...
                .AnyAsync(
                    employee =>
                        employee.Id == request.CurrentCustodianId.Value &&
                        employee.IsActive,
                    cancellationToken);
    ...
    private async Task ValidateCategoryAsync(
        ...
        CancellationToken cancellationToken)
    {
        var isValid = await _dbContext.AssetCategories
            .AsNoTracking()
            .AnyAsync(
                category =>
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var normalizedBarcode = TextNormalizer.Required(barcodeValue);

            var exists = await _dbContext.Assets
                .AsNoTracking()
                .AnyAsync(
                    asset =>
                        asset.BarcodeValue == normalizedBarcode &&
        ...
        var normalizedSerial = TextNormalizer.Optional(serialNumber);
        ...
        var serialExists = await _dbContext.Assets
            ...
            .AnyAsync(
                asset =>
                    asset.SerialNumber == normalizedSerial &&
                    (!excludedAssetId.HasValue ||
                     asset.Id != excludedAssetId.Value),
                cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            command.CommandText = """SELECT nextval('"AssetIdSequence"');""";
